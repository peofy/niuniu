...
class Member extends Common
{
    ...
    public function index()
    {
        ...
        if ($tplType == 'export') {
            ...
            ->select();
                ...
                $strTable .= '<td style="text-align:center;font-size:12px;" width="200">推荐人ID</td>';
                ...
                // $strTable .= '<td style="text-align:center;font-size:12px;" width="*">佣金累计收益</td>';
                ...
                foreach($list as $lk=>$gv){
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;width:auto;">&nbsp;'.$gv['first_leader'].' </td>';	
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$gv['realname'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.memberlevel($gv['level']).'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$gv['mobile'].'</td>';
                    // $strTable .= '<td style="text-align:left;font-size:12px;">'.$gv['distribut_money'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$gv['remainder_money'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.date("Y-m-d H:i:s",$gv['createtime']).'</td>';
                
                    $strTable .= '</tr>';
                }
                $strTable .='</table>';
                unset($list);
                downloadExcel($strTable,'会员列表');
                exit();
            }
    ...
    public function level()
    {
        ...
        //     $list[$lk]['upgrade_bonus']=$lv['upgrade_bonus']*1;
        // }
        $this->assign('list', $list);
    ...
    public function level_edit()
    {
        ...
        // $info['upgrade_bonus']=$info['upgrade_bonus']*1;
        ...
        if (request()->isPost()) {
            ...
            $desc  = input('desc', '');
            $team_num=input('team_num',0);
            $agent_amount=input('agent_amount',0);
            $agent_bonus=input('agent_bonus',0);
            $direct_rate=input('direct_rate',0);
            $indirect_rate=input('indirect_rate',0);
            // $upgrade_bonus=input('upgrade_bonus',0);
            ...
            // $data['upgrade_bonus']=$upgrade_bonus;
            $data['desc']  = $desc;
            $data['team_num']=$team_num;
            $data['agent_amount']=$agent_amount;
            $data['agent_bonus']=$agent_bonus;
            $data['direct_rate']=$direct_rate/100;
            $data['indirect_rate']=$indirect_rate/100;
