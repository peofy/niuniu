...
use app\common\model\Youxuan as Youxuan;
...
class Index extends Common
{
     ...
     * 优选单品
     */
    public function youxuanlist(){
        $list = Db::table('youxuan')->where(['status'=>['<>',-1]])->select();
        $this->assign('list', $list);
        $this->assign('meta_title', '优选单品');
        return $this->fetch();
    }


    public function youxuanedit(){
        ...
        if (request()->isPost()) {
            ...
            !$title && $this->error('标题不能为空');
            // 图片验证
            $res = Youxuan::pictureUpload('fixed_picture', 0);
            if ($res[0] == 1) {
                $this->error($res[0]);
            } else {
                $pictureName                             = $res[1];
            ...
            if ($id) {
                $Catenav = new Youxuan;
                if ($Catenav->save($data, ['id' => $id]) !== false) {
                    $this->success('编辑成功', url('index/youxuanlist'));
                }
                $this->error('编辑失败');
            }
            ...
            $file = request()->file('file');
            !$file && $this->error('图片不能为空');
            $Catenav = new Youxuan($data);
            if ($Catenav->save()) {
                $this->success('添加成功', url('index/youxuanlist'));
            }
            $this->error('添加失败');
        }
        $info = $id ? Youxuan::where('id', $id)->find()->getdata() : [];
        $this->assign('info', $info);
        $this->assign('id', $id);
        $this->assign('meta_title', $id ? '编辑优选单品' : '新增优选单品');
        return $this->fetch();
    }
    public function updateyouxuan_status(){
        $id = request()->param('id',0);
        $status = request()->param('status',0);
        if ($id){
            $update = Db::table('youxuan')->where('id',$id)->update(['status'=>$status]);
            if ($update){
                return json(['code'=>1,'msg'=>'操作成功！','data'=>[]]);
            }else{
                json(['code'=>0,'msg'=>'修改失败！','data'=>[]]);
    ...
    public function delyouxuan(){
        $id = input('id', 0);
        if (Db::table('youxuan')->where('id', $id)->delete()) {
            $this->success('删除成功！');
        }
        $this->error('删除失败！');
    }
