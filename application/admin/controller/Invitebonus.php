...
class Invitebonus extends Common
{
    /**
     * 会员列表
     */
    ...
    public function invite_bonus()
    {
        $list = Db::name('invite_bonus')->select();
        $this->assign('list', $list);
        $this->assign('meta_title', '邀请奖励');
        return $this->fetch();
    }
    ...
    {
        $id   = input('id', 0);
        $info = Db::table('invite_bonus')->where('id', $id)->find();
        if (request()->isPost()) {

            $pop_num      = input('pop_num');
            $team_num      = input('team_num');
            $per_invite_bonus       = input('per_invite_bonus');
            // $bonus       = input('bonus');
            $level       = input('level');
            // if (empty($levelname)) {
            //     $this->error('等级名称不能为空！');
            ...
            $data['per_invite_bonus']   = $per_invite_bonus;
            // $data['bonus']   = $bonus;
            $data['level']   = $level;
            $res = Db::table('invite_bonus')->where(['id' => $id])->update($data);
            if ($res !== false) {
                $this->success('操作成功', url('invitebonus/invite_bonus'));
            } else {
                $this->error('失败！');
            }
        };
