...
class Order extends ApiBase
{
    ...
    public function temporary()
    {
        ...
        $addr_res['address'] = $addr_res['p_cn'] . $addr_res['c_cn'] . $addr_res['d_cn'] . $addr_res['s_cn'] . $addr_res['address'];
               foreach ($cart_res as $key => $value){
                   $stock = Db::table('goods')->field('stock')->where('goods_id',$value['goods_id'])->find()['stock'];

                   $cart_res[$key]['stock'] = $stock;
               }
        ...
        foreach($data['goods_res'] as $key=>&$value){
            ...
            $value['rmb_price']=$value['goods_price']*$this->aus_tormb;
    ...
    {   
        ...
        //判断如果商品是进口商品和没有上传身份证就拒绝提交订单

        $onecart=Db::name('cart')->where(['id'=>$cart_str])->find();
        $onegoods=Db::name('goods')->where(['goods_id'=>$onecart['goods_id']])->find();

        $oneuser=Db::name('member')->where(['id'=>$user_id])->find();
        
        if(($onegoods['cat_id1']==13)){
            if(empty($oneuser['pic_front'])||empty($oneuser['pic_back'])||empty($oneuser['name'])||empty($oneuser['idcard']) ){
                $this->ajaxReturn(['status' => 301 , 'msg'=>'进口商品需要填写身份证信息','data'=>'']);
            }
        }
...
//             }
        ...
        foreach($order_goods as $key=>$value){
            ...
            $push = Db::table('goods')
                    ...
                    ->field('is_push,price')
                    ->find();
            $mouthpush = Db::table('cart')
                    ->where('goods_id', $value['goods_id'])
                    ->where('id','in',$cart_str)
                    ->field('goods_price')
                    ->find();
            ...
            if ($push['is_push'] == 1){
                ...
                $pool['goods_id'] =  $value['goods_id'];
                $pool['uid']      = $user_id;
                $pool['money']    = $mouthpush['goods_price'] * ($money['value']/100);
                $pool['is_day']   = 1;
                $pool['create_time'] = time();
                $res = Db::table('bonus_pool')->insert($pool);
                ...
                $nickname = Db::table('member')->where('id',$user_id)->field('realname')->find();
                $log['uid'] = $user_id;
                $log['title'] = $nickname['realname'].'购买产品增加月奖金池'.$mouthpush['goods_price'] * ($money['value']/100) .'元';
                $log['money'] = $mouthpush['goods_price'] * ($money['value']/100);
                $log['create_time'] = time();
                $log['is_day'] = 1;
                Db::table('cashing_prize_log')->insert($log);
...
//        print_r($where);die;
        ...
        if($order_list){
            ...
            foreach($order_list as $key=>&$value){
                ...
                // $order_list[$key]['rmb_price']=$value['price']*$this->aus_tormb;
                ...
                foreach($value['goods'] as $k=>$v){
                    ...
                    $value['goods'][$k]['img'] =SITE_URL.'/public/upload/images/'. $value['goods'][$k]['img'];
    ...
    public function order_detail()
    {
        ...
        foreach($order['goods_res'] as $key=>&$value){
            ...
            $order['goods_res'][$key]['img'] =SITE_URL.'/public/upload/images/'. Db::table('goods_img')->where('goods_id',$value['goods_id'])->where('main',1)->value('picture');
        }
        ...
        $is_pwd = Db::name('member')->where(['id' => $user_id])->field('pwd,remainder_money,alipay,alipay_name')->find();
        if ($is_pwd['pwd']){
            $order['is_pwd']=1;
        }else{
            $order['is_pwd']=0;
  ...
  */
    public function order_pay(){
        ...
        if($pay_type==1||$pay_type==4){
            ...
            if(!empty($pwd) && ($member['pwd'] != md5(C("AUTH_CODE").$pwd))){
...
//            $comments[0]['img'] = implode(',',$comments[0]['img']);
        ...
        $file1 = request()->file('img1');//正面
        if (!empty($file1)){
            // 移动到框架应用根目录/uploads/ 目录下
            $info1 = $file1->validate(['size'=>2048000000,'ext'=>'jpg,png,gif']);
            $info1 = $file1->rule('md5')->move(ROOT_PATH . DS.'public/upload/images');//加密->保存路径
            if($info1){
                // 成功上传后 获取上传信息
                // 输出 jpg
                ...
                $img1 = $info1->getSaveName(); //输出路径
                // ROOT_PATH . DS.
                // 存着 地址
            }else{
                $this->ajaxReturn(['status' => -2 , 'msg'=>'上传失败1','data'=>$file1->getError()]);
            }

        }
        $file2 = request()->file('img2');//正面
        if (!empty($file2)){
            // 移动到框架应用根目录/uploads/ 目录下
            $info2 = $file2->validate(['size'=>2048000000,'ext'=>'jpg,png,gif']);
            $info2 = $file2->rule('md5')->move(ROOT_PATH . DS.'public/upload/images');//加密->保存路径
            if($info2){
                // 成功上传后 获取上传信息
                // 输出 jpg
                // echo $info->getExtension();
                ...
                // echo $info->getSaveName();
                // 输出 42a79759f284b767dfcb2a0197904287.jpg
                $img2 = $info2->getSaveName(); //输出路径
                // ROOT_PATH . DS.
                // 存着 地址
            }else{
                $this->ajaxReturn(['status' => -2 , 'msg'=>'上传失败2','data'=>$file2->getError()]);
            }

        }
        $file3 = request()->file('img3');//正面
        if (!empty($file3)){
            // 移动到框架应用根目录/uploads/ 目录下
            $info3 = $file3->validate(['size'=>2048000000,'ext'=>'jpg,png,gif']);
            $info3 = $file3->rule('md5')->move(ROOT_PATH . DS.'public/upload/images');//加密->保存路径
            if($info3){
                // 成功上传后 获取上传信息
                // 输出 jpg
                // echo $info->getExtension();
                ...
                // echo $info->getSaveName();
                // 输出 42a79759f284b767dfcb2a0197904287.jpg
                $img3 = $info3->getSaveName(); //输出路径
                // ROOT_PATH . DS.
                // 存着 地址
            }else{
                $this->ajaxReturn(['status' => -2 , 'msg'=>'上传失败3','data'=>$file3->getError()]);
            }

        }
        if (!empty($img1)){
            $comments[0]['img'][1] = $img1;
        }
        if (!empty($img2)){
            $comments[0]['img'][2] = $img2;
        }
        if (!empty($img3)){
            $comments[0]['img'][3] = $img3;
        }
...
//            Db::table('order_goods')->where('order_id',$order_id)->where('sku_id',$sku_id)->update(['is_comment'=>1]);
            Db::commit();
            $this->ajaxReturn(['status' => 200 , 'msg'=>'成功！','data'=>$comments]);
        }
        Db::rollback();
