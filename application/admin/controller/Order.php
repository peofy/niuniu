   ...
   /**
    ...
    public function index()
    {
        ...
        if (!empty($order_sn)) {
            ...
            $exportParam['order_sn']=$order_sn;
        }
        if (!empty($invoice_no)) {
            $where['d.invoice_no']   = $invoice_no;
            $exportParam['invoice_no']=$invoice_no;
        }
        ...
        if ($tplType == 'export') {
            ...
            ->select();
                ...
                $strTable .= '<td style="text-align:center;font-size:12px;" width="200">订单号</td>';
                ...
                foreach($list as $lk=>$gv){
                    $strTable .= '<tr>';
                    $strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$gv['order_id'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;width:auto;">&nbsp;'.$gv['order_sn'].' </td>';	
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$gv['order_amount'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.type_name($gv['pay_type']).'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$order_status[$gv['order_status']].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.date("Y-m-d H:i:s",$gv['add_time']).'</td>';
               
                    $strTable .= '</tr>';
                }
                $strTable .='</table>';
                unset($list);
                downloadExcel($strTable,'订单列表');
                exit();
    ...
    public function edit(){
        ...
        $memberinfo=Db::name('member')->where(['id'=>$order_info['user_id']])->find();
...
//                }
    ...
    public function delivery_info($id=''){
        ...
        $memberinfo=Db::name('member')->where(['id'=>$orderObj['user_id']])->find();
