...
//记入日志
function write_log($content,$name)
{
...
function type_name ($pay_type){
    $type_name=Config("pay_kind.{$pay_type}");
    return $type_name;
}
...
{ //掌骏
    ...
    $content = "【牛项目】您的手机验证码为：" . $code . "，该短信1分钟内有效。如非本人操作，可不用理会！";
    $time = date('ymdhis', time());
    //    $arr=array('uname'=>"hsxx40",'pwd'=>"hsxx40",'time'=>$time);
    $arr = array('uname' => "ZFKJ10100", 'pwd' => "123456789", 'time' => $time);
    $signPars = '';
    foreach ($arr as $v) {
        $signPars .= $v;
    }
    ...
    $arrs = array('userid' => "2582", 'timestamp' => $time, 'sign' => $sign, 'mobile' => $mobile, 'content' => $content, 'action' => 'send');
...
function isMobile($mobile)
{
    ...
    return preg_match('#^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,6,7,8]{1}\d{8}$|^18[\d]{9}$|^19[8,9]{1}\d{8}$#', $mobile) ? true : false;
}
...
function memberlevel($level){
    if($level>0){
         ...
         $member_level=$member_level[$level]['levelname'];
    }else{
        $member_level='未绑定推荐人';
    }
    return $member_level;
 ...
 * @return bool
 */
function check_idcard($idcard,$realname,$user_id)
{   
    //配置您申请的appkey
    $appkey = "2c057cc3b290263df1dc0e75afb6721d";
    ...
    $member_nameapi = Db::name('member_nameapi')->where(['idNum'=>$idcard,'user_id'=>$user_id,'name'=>$realname])->find();

    if(!empty($member_nameapi)){
        return true;
    }else{
        ...
        if($result){
            if($result['error_code']=='0'){
                if($result['result']['res'] == '1'){
                    ...
                    Db::name('member_nameapi')->insert($data);
                    return true;
                }else{
                    // echo "身份证号码和真实姓名不一致";
                    return false;
