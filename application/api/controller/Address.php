...
//        }
    ...
    {
        if(!Request::instance()->isPost()){
            return $this->failResult("请求方式错误" ,301);
        }
        $data=input('post.');
        // $user_id=42;//测试
        $user_id=$this->get_user_id();
        ...
        if (empty($data['address_id'])){
            ...
            if (empty($data['district'])){
                $res = Db::table('user_address')->where('address_id',$address_id)->find()['district'];
                $data['district'] = Db::table('region')->where('area_id',$res)->find()['code'];
            }
        }
        $useraddr=new UserAddr();
    ...
    public function my_address()
    {
        ...
        if(!empty($my_address)){
            $my_address['districtcode']=Db::name('region')->field('code')->where('area_id',$my_address['district'])->find()['code'];
            $my_address['countryname']=Db::name('region')->field('area_name')->where('area_id',$my_address['country']+1)->find()['area_name'];
            $my_address['provincename']=Db::name('region')->field('area_name')->where('area_id',$my_address['province'])->find()['area_name'];
            $my_address['cityname']=Db::name('region')->field('area_name')->where('area_id',$my_address['city'])->find()['area_name'];
            $my_address['districtname']=Db::name('region')->field('area_name')->where('area_id',$my_address['district'])->find()['area_name'];
            $my_address['twonname']=Db::name('region')->field('area_name')->where('area_id',$my_address['twon'])->find()['area_name'];
    ...
    public function set_default_address()
    {
        ...
        $address_id=input("address_id");
        if(!$address_id){
            return $this->failResult("地址id不能为空",301);
        }
        ...
        if($setDefault){
            ...
            return $this->failResult("操作失败",301);
        }
    } 
