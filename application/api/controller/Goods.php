...
class Goods extends ApiBase
{
    ...
    */
    ...
    public function categoryList()
    {
        ...
        foreach($list as $key=>$value){
            $list[$key]['goods'] = Db::table('goods')->alias('g')
                                ...
                                ->join('goods_img gi','gi.goods_id=g.goods_id','LEFT')
                                ->order('g.goods_id DESC')
                                ->field('g.goods_id,goods_name,cat_id2,gi.picture img,price,original_price,GROUP_CONCAT(ga.attr_name) attr_name,g.cat_id1 comment')
                                ->select();
            if($list[$key]['goods']){
                foreach($list[$key]['goods'] as $k=>$v){
                    ...
                    $list[$key]['goods'][$k]['img'] =SITE_URL.'/public/upload/images/'.$v['img'];
                    $list[$key]['rmb_price']=$v['price']*$this->aus_tormb;
    ...
    public function cat2goods()
    {
        ...
        if($goods){
            foreach($goods as $k=>$v){
                if($v['attr_name']){
                    $goods[$k]['attr_name'] = explode(',',$v['attr_name']);
                }else{
                    $goods[$k]['attr_name'] = array();
                }

                $goods[$k]['img'] = SITE_URL.'/public/upload/images/' .$v['img'];
                $goods[$k]['rmb_price']=$v['price']*$this->aus_tormb;
                $goods[$k]['comment'] = Db::table('goods_comment')->where('goods_id',$v['goods_id'])->count();
            }
        }
        $this->ajaxReturn(['status' => 200 , 'msg'=>'获取成功','data'=>$goods]);
    }
    ...
    public function goodsDetail()
    {
        ...
        $goodsRes['rmb_price']=$goodsRes['price']*$this->aus_tormb;
        ...
        if($goodsRes['img']){
            foreach($goodsRes['img'] as $key=>&$value){
                ...
                $value['picture'] = SITE_URL.'/public/upload/images/'.$value['picture'];
            }
        }
//        array_pop($goodsRes['img']);
        ...
        $comment = Db::table('goods_comment')->alias('gc')
            ...
            ->field('m.id,m.avatar,m.realname,gc.content,gc.star_rating,gc.replies,gc.praise,gc.add_time,gc.img,gc.sku_id')
...
//        $goodsRes['address'] = $addr_res['p_cn'] . $addr_res['c_cn'] . $addr_res['d_cn'] . $addr_res['s_cn'] . $addr_res['address'];
    ...
    public function search_goods()
    {
        ...
        foreach($list as $k=>$v){
            ...
            $list[$k]['rmb_price']=$v['price']*$this->aus_tormb;
            $v['comment_num'] = 0;
            $v['goods_num'] =0;
    ...
    * }
    ...
    public function comment_list(){
        ...
        $comment = Db::table('goods_comment')->alias('gc')
                ->join('member m','m.id=gc.user_id','LEFT')
                ->field('m.mobile,m.realname,m.avatar,gc.user_id,gc.id comment_id,gc.content,gc.star_rating,gc.replies,gc.praise,gc.add_time,gc.img,gc.sku_id')
                ->order('gc.add_time DESC')
                ->where('gc.goods_id',$goods_id)
                ->paginate($pageArray)
                ->toArray();
...
//        $comment = $comment->all();
        ...
        foreach($comment['data'] as $key=>$value ){
            ...
            $comment['data'][$key]['mobile'] = $value['mobile'] ? substr_cut($value['mobile']) : '';

            if($value['img']){
                $comment['data'][$key]['img'] = explode(',',$value['img']);
                foreach($comment['data'][$key]['img'] as $k=>&$v){
                    $comment['data'][$key]['img'][$k] = SITE_URL.'/public/upload/images/' .$v;
                }
            }else{
                $comment['data'][$key]['img'] = [];
            }

            $comment['data'][$key]['spec'] = $this->get_sku_str($value['sku_id']);
            $comment['data'][$key]['add_time'] =date('Y-m-d H:i:s',$comment['data'][$key]['add_time']);

            $comment['data'][$key]['is_praise'] = Db::table('goods_comment_praise')->where('comment_id',$value['comment_id'])->where('user_id',$user_id)->count();
        }
    ...
    public function getGoodsSpec($goods_id){
        ...
        foreach ($skuRes as $sku_k=>$sku_v){
            ...
            $skuRes[$sku_k]['rmb_price']=$sku_v['price']*$this->aus_tormb;
